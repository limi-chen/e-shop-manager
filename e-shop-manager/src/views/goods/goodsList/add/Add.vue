<template>
	<div>
				...
				<el-tabs v-model="activeIndex" :tab-position="'left'"
				  ...
				  <el-tab-pane label="基本信息" name="0">
				  	...
				  	<el-form-item label="商品分类" prop="goods_cat">
				  		<el-cascader v-model="addForm.goods_cat" :options="cateList" :props="{ expandTrigger: 'hover', value: 'cat_id', label: 'cat_name',children: 'children' }"
		...
		<el-dialog title="图片预览" :visible.sync="previewVisible" width="50%" >
		  <img :src="previwePath" class="previewImg" />
		</el-dialog>
	...
	export default {
		...
		data(){
			return {
				...
				uploadURL: 'http://127.0.0.1:8888/api/private/v1/upload',   //上传路径
				//图片上传组件的headersqi请求头对象
				headersObj: {
		...
		methods: {
			...
			handlePreview(file) {
				console.log(file)
				this.previwePath = file.response.data.url
			...
			handleRemove(file) {
				...
				//2. 从 pic 数组中，找到这个图片对应的索引值
				const i = this.addForm.pics.findIndex(x => {
					x.pic === filePath
				})
				//3. 调用数组的 split 方法，把图片信息对象，从 pics 数组中移出
